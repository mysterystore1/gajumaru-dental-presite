...
### 4-4. 要判断3件（★**オーナー確認が取れてから**着手・現状保留）
- **ふきだし化**（dental-anxiety内心セリフ）：1種類・控えめ前提。Opus見解=発話の記号で箱化と別物だが境界。→ オーナー判断待ち。
- **CTA帯リッチ化の枠先行**：承認済み。ただし**電話番号の大型化は実NAP確定後**。枠だけ先行可。
- **「がじゅまる=家族を守る木」由来の掲載**：（仮）で枠先行を推奨、院長の言葉確定後に差替。→ オーナー判断待ち。

> 進め方：**4-1 → 4-2 → 4-3 を先行**（1コミットずつ・実機確認）。**4-4 はオーナー確認後**。Phase完了で push → Opus へ報告 → 受領検証 → 次へ。
