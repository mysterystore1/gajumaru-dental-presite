...
- [ ] 🎪 **内覧会 7/25(土)・26(日) を確定**（DEC-012・仮確定）：桑田に「設備readiness（内装完成・チェア設置・要なら保健所検査）が間に合うか」最終確認→OKでチラシ入稿・サイト時間追記。サイトは「開催予定」で仮入れ済（ルートindex.html）。⚠チラシのフォトリアル医師写真はDEC-009懸念＝差し替え検討
